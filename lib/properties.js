...
internals.properties.prototype.parseProperty = function(name, joiObj, parent, parameterType, useDefinitions, isAlt) {
  ...
  //TODO: this no longer returns what we need with the where() option
  let joiType = joiObj.type.toLowerCase();

  if (Hoek.reach(joiObj, '$_terms.whens') !== null) {
    // console.log('joiObj contains whens', Hoek.reach(joiObj, '$_terms.whens'))
    //Override the joiType to `alternatives`, since joiObj.type doesn't return alternatives as of Joi 16
    console.log('found alternatives, Overriding joiType.')
    // joiType = 'alternatives'
  }
  ...
  if (property.type === 'object') {
    //TODO: maybe we need to check for a where here?
  ...
  // I think this is the problem, as the when() no longer returns an `alternatives` property type
  // We need to find a way to figure out if a property is an alternatives
...
internals.properties.prototype.parseAlternatives = function(property, joiObj, name, parameterType, useDefinitions) {
  ...
  //In joi 16, the when() implementation has changed a fair bit, breaking some of these assumptions
  ...
  if (Hoek.reach(joiObj, '$_terms.matches.0.schema')) {
  ...
  else if (Hoek.reach(joiObj, '$_terms.whens.0.then')) {
    // add first into definitionCollection

    let child = joiObj.$_terms.whens[0].then;
    ...
    // create the alternatives without appending to the definitionCollection
    if (property && this.settings.xProperties === true) {
      let altArray = joiObj.$_terms.whens
        .reduce((res, obj) => {
    ...
    console.log('terms are', joiObj.$_terms)
