...
builder.getSwaggerJSON = async (settings, request) => {
  ...
  //routes are even bad here - routes[n].settings.validate.pathParams is `null`
  //I think this is related to a different library change maybe?
  let routes = request.server.table();
  
  routes = Filter.byTags(['api'], routes);
  console.log('basic routes after filtering', routes[0].settings.validate)
